...
class infoNCELoss(nn.Module):
    ...
    def __init__(self, margin=0,temp=1, measure=False, max_violation=False, cost_style='sum', direction='all'):
    ...
    def forward(self, s, im):
        ...
        cost = None
        cost = torch.exp(scores / self.temp)
        diagonal = cost.diag().view(im.size(0),1)
        cost=cost.sum(axis=1).view(im.size(0),1)
        if cost is None:
            cost = Variable(torch.zeros(1)).cuda()
        result=-1*torch.log(diagonal/cost)
        return result.sum()
